...
void DecrementExprNode::codegen(nv::IRGenerationContext& ctx) {
    ...
    if (auto* id = dynamic_cast<IdentifierNode*>(operand.get())) {
        ...
        auto* bbInt = llvm::BasicBlock::Create(c, "dec.int", curFn);
        auto* bbFloat = llvm::BasicBlock::Create(c, "dec.float", curFn);
        auto* bbMerge = llvm::BasicBlock::Create(c, "dec.merge", curFn);
        ...
        auto* bbArr = llvm::BasicBlock::Create(c, "dec.set.array", curFn);
        auto* bbVec = llvm::BasicBlock::Create(c, "dec.set.vector", curFn);
        auto* bbSetMerge = llvm::BasicBlock::Create(c, "dec.set.merge", curFn);
